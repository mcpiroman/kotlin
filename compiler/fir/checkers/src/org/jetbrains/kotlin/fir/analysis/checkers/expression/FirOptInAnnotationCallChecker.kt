        ...
        if (isMarker || isOptIn) {
            ...
            if (isOptIn) {
                ...
                if (arguments.isEmpty()) {
                    ...
                    val unwrappedArguments = (arguments.first() as? FirVarargArgumentsExpression)?.arguments.orEmpty()
                    for (argument in unwrappedArguments) {
                        if (argument is FirGetClassCall) {
                            val classSymbol = (argument.argument as? FirResolvedQualifier)?.symbol as? FirRegularClassSymbol
                            with(FirOptInUsageBaseChecker) {
                                if (classSymbol != null && classSymbol.fir.loadExperimentalityForMarkerAnnotation() == null) {
                                    reporter.reportOn(
                                        expression.source,
