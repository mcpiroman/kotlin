...
interface BirTypeSystemContext : TypeSystemContext, TypeSystemCommonSuperTypesContext, TypeSystemCommonBackendContext {
    ...
    val birTreeContext: BirTreeContext // not as a contextual interface due to https://youtrack.jetbrains.com/issue/KT-51881/Consider-proper-support-for-context-receivers-in-interfaces
    ...
    private fun getTypeParameters(typeConstructor: TypeConstructorMarker): List<BirTypeParameter> {
        return when (typeConstructor) {
            ...
            is BirClassSymbol -> with(birTreeContext) { extractTypeParameters(typeConstructor.asElement) }
    ...
    override fun captureFromArguments(type: SimpleTypeMarker, status: CaptureStatus): SimpleTypeMarker? {
        ...
        val typeParameters = with(birTreeContext) { extractTypeParameters(element) }
    ...
    override fun KotlinTypeMarker.getAnnotationFirstArgumentValue(fqName: FqName): Any? =
        (this as? BirType)?.annotations?.firstOrNull { annotation ->
            annotation.target.asElement.parentAsClass.hasEqualFqName(fqName)
        }?.run {
    ...
    private fun TypeConstructorMarker.getNameForClassUnderKotlinPackage(): String? {
        ...
        return if (signature != null) {
            ...
            if (parent is BirPackageFragment && parent.fqName == StandardNames.BUILT_INS_PACKAGE_FQ_NAME)
    ...
    override fun TypeConstructorMarker.isUnderKotlinPackage(): Boolean {
        ...
        return declaration.ancestors()
            ...
            .any { (it.parent as? BirPackageFragment)?.fqName?.startsWith(StandardNames.BUILT_INS_PACKAGE_NAME) == true }
    }
    ...
    override fun substitutionSupertypePolicy(type: SimpleTypeMarker): TypeCheckerState.SupertypesPolicy {
        require(type is BirSimpleType)
        val parameters = with(birTreeContext) { extractTypeParameters(type.classifier as BirClass).memoryOptimizedMap { it.asElement } }
...
class BirTypeSystemContextImpl(
    ...
    override val birTreeContext: BirTreeContext
